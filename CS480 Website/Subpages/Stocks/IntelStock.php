<?php
	...
	$json = file_get_contents('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=INTC&apikey=I19RPNEJSJH056FK');

	$data = json_decode($json, true);
						
	// check if the array key exists
	if (!empty($data['Time Series (Daily)']) && !isset($_COOKIE['Intel'])) {
				
		// Remove all old stock data
		$sql = "DELETE FROM stock WHERE company_ID = 4;";
		$conn->query($sql);
		...
		foreach( $data['Time Series (Daily)'] AS $date => $results ) { 
			...
			$sql = "REPLACE INTO stock SET company_ID = 4, date = '$date', time = NULL, open = '$open', high = '$high', low = '$low', close = '$close', volume = '$volume';";
			$conn->query($sql);
					
			// Use a cookie to prevent stock price updates for 5 minutes to limit API calls
			setcookie("Intel", 'updated', time() + 300000);
			$_COOKIE['Intel'] = 'updated';
		}
	}
			
	$sql = "SELECT * FROM stock WHERE company_ID = 4 ORDER BY date ASC;";

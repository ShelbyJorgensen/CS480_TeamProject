<?php
	...
	$json = file_get_contents('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=TSLA&apikey=I19RPNEJSJH056FK');

	$data = json_decode($json, true);
						
	// check if the array key exists
	if (!empty($data['Time Series (Daily)']) && !isset($_COOKIE['Tesla'])) {
				
		// Remove all old stock data
		$sql = "DELETE FROM stock WHERE company_ID = 8;";
		$conn->query($sql);
		...
		foreach( $data['Time Series (Daily)'] AS $date => $results ) { 
			...
			$sql = "REPLACE INTO stock SET company_ID = 8, date = '$date', time = NULL, open = '$open', high = '$high', low = '$low', close = '$close', volume = '$volume';";
			$conn->query($sql);
					
			// Use a cookie to prevent stock price updates for 5 minutes to limit API calls
			setcookie("Tesla", 'updated', time() + 300000);
			$_COOKIE['Tesla'] = 'updated';
		}
	}
			
	$sql = "SELECT * FROM stock WHERE company_ID = 8 ORDER BY date ASC;";
